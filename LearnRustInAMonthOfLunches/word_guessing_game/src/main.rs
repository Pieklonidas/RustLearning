...
impl GameApp {
    fn restart(&mut self) {
        self.current_word = RANDOM_WORDS[fastrand::usize(..RANDOM_WORDS.len())]
            .to_lowercase();
            ...
            guess if guess.chars().count() == 1 => {
                ...
                match self.check_guess(the_guess) {
                    ...
                    Guess::Right => {
                        ...
                        output.push_str(&format!("Yes, it contains {the_guess}!\n"));
                    },
                    Guess::Wrong => {
...
async fn main() {
    ...
    axum::Server::bind(&"127.0.0.1:50080".parse().unwrap())
